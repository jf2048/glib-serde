...
use glib_serde::{prelude::*, from_variant, to_variant, Variant};
...
fn serialize_structs() {
    ...
    let variant = to_variant(&MyWrapperStruct {
        ...
        sig: MyNewtypeStruct::static_variant_type().deref().try_into().unwrap(),
...
fn deserialize_structs() {
    ...
    assert_eq!(value.dict.lookup_value("hello", None).unwrap().str().unwrap(), "world");
}
